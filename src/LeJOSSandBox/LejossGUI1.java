...
public class LejossGUI1 {
	...
	private void initialize() {
		...
		formattedTextField.setValue(getFloatValue());
		 formattedTextField.getValue();
		//formattedTextField.g
		// formattedTextField.setValue(setFloatValue(0));
	...
	public int getFloatValue() {
    	...
    	IntW parameterValue = new IntW(0);
		handle = new IntW(0);
		...
		error = VrepConnectionFactory.getInstance().VrepConnection_get_object().simxGetObjectIntParameter(VrepConnectionFactory.getInstance().VrepConnection_get_ID(), handle.getValue(), parameterID, parameterValue, VrepConnectionFactory.getInstance().VrepConnection_get_object().simx_opmode_streaming);
		try {
		...
		error = VrepConnectionFactory.getInstance().VrepConnection_get_object().simxGetObjectIntParameter(VrepConnectionFactory.getInstance().VrepConnection_get_ID(), handle.getValue(), parameterID, parameterValue, VrepConnectionFactory.getInstance().VrepConnection_get_object().simx_opmode_buffer);
		//System.out.println(parameterValue.getValue());
